...
def detect_rsi_crossover(df):
    ...
    # 15M OPTIMIZED volume filter - much more permissive for 15-minute signals
    if curr['volume_ratio'] < 0.4:  # Volume must be at least 40% of average (15m optimized)
        logger.info(f"❌ VOLUME FILTER: Volume too low ({curr['volume_ratio']:.2f} < 0.4)")
        return None
    
    logger.info(f"✅ VOLUME PASSED: {curr['volume_ratio']:.2f} >= 0.4")
    
    # 15M OPTIMIZED Buy Signal - wider RSI ranges perfect for 15-minute timeframe
    if (prev2['rsi_9'] < prev2['rsi_21'] and 
        prev['rsi_9'] <= prev['rsi_21'] and 
        curr['rsi_9'] > curr['rsi_21'] and
        20 < curr['rsi_9'] < 75 and 25 < curr['rsi_21'] < 70 and  # 15M optimized ranges
        curr['rsi_9'] > prev['rsi_9']):  # RSI-9 trending up
        return 'BUY'
    
    # 15M OPTIMIZED Sell Signal - wider RSI ranges perfect for 15-minute timeframe
    if (prev2['rsi_9'] > prev2['rsi_21'] and 
        prev['rsi_9'] >= prev['rsi_21'] and 
        curr['rsi_9'] < curr['rsi_21'] and
        25 < curr['rsi_9'] < 80 and 30 < curr['rsi_21'] < 75 and  # 15M optimized ranges
        curr['rsi_9'] < prev['rsi_9']):  # RSI-9 trending down
        logger.info(f"✅ SELL CROSSOVER DETECTED - RSI9: {curr['rsi_9']:.1f} < RSI21: {curr['rsi_21']:.1f}")
        return 'SELL'
    
    logger.info(f"❌ NO CROSSOVER - RSI9: {curr['rsi_9']:.1f}, RSI21: {curr['rsi_21']:.1f}")
    return None

def confirm_with_macd(df, signal_type, symbol):
    """15M OPTIMIZED MACD confirmation - much more permissive for 15-minute signals"""
    if len(df) < 2:
        return False
    ...
    # 15M OPTIMIZED strength requirements - much more permissive
    if 'DOGE' in symbol or 'ADA' in symbol or 'TRX' in symbol:
        min_strength = 0.00005  # Very low for altcoins
    elif 'BTC' in symbol:
        min_strength = 2        # Much lower for BTC
    elif 'ETH' in symbol:
        min_strength = 0.2      # Much lower for ETH
    else:
        min_strength = 0.05     # Much lower for other pairs
    
    logger.info(f"📊 MACD CHECK [{symbol}] - Line: {curr['macd_line']:.6f}, Signal: {curr['macd_signal']:.6f}, Strength: {abs(curr['macd_line']):.6f} (min: {min_strength})")
    
    # 15M OPTIMIZED: Much more flexible MACD logic
    if signal_type == 'BUY':
        macd_ok = (curr['macd_line'] > curr['macd_signal'] or  # Traditional crossover OR
                   curr['macd_line'] > prev['macd_line']) and abs(curr['macd_line']) > min_strength  # Rising momentum
        
        if macd_ok:
            logger.info(f"✅ MACD BUY CONFIRMED")
        else:
            logger.info(f"❌ MACD BUY REJECTED")
        return macd_ok
    
    elif signal_type == 'SELL':
        macd_ok = (curr['macd_line'] < curr['macd_signal'] or  # Traditional crossover OR
                   curr['macd_line'] < prev['macd_line']) and abs(curr['macd_line']) > min_strength  # Falling momentum
        
        if macd_ok:
            logger.info(f"✅ MACD SELL CONFIRMED") 
        else:
            logger.info(f"❌ MACD SELL REJECTED")
        return macd_ok
...
def additional_market_filters(df, signal_type):
    """15M OPTIMIZED filters - much more permissive for 15-minute signal generation"""
    curr = df.iloc[-1]
    closes = df['close']
    highs = df['high']
    lows = df['low']

    logger.info(f"🔍 15M MARKET FILTERS CHECK for {signal_type}")

    # 15M OPTIMIZED trend filter - much more flexible
    ema20 = closes.ewm(span=20, adjust=False).mean().iloc[-1]
    ema_trend_strength = (ema20 - closes.ewm(span=20, adjust=False).mean().iloc[-10]) / closes.ewm(span=20, adjust=False).mean().iloc[-10]
    
    if signal_type == 'BUY':
        trend_ok = curr['close'] >= ema20 * 0.995  # Much more flexible - 0.5% below EMA allowed
        ema_ok = ema_trend_strength >= -0.002      # Allow slight downtrend
    else:
        trend_ok = curr['close'] <= ema20 * 1.005  # Much more flexible - 0.5% above EMA allowed  
        ema_ok = ema_trend_strength <= 0.002       # Allow slight uptrend
    
    logger.info(f"📈 TREND: Price vs EMA20: {((curr['close']/ema20-1)*100):+.2f}%, EMA strength: {(ema_trend_strength*100):.3f}%")
    
    if not trend_ok or not ema_ok:
        logger.info(f"❌ TREND REJECTED")
        return False

    # 15M OPTIMIZED volatility - much lower threshold
    volatility = highs.rolling(10).std() / closes.rolling(10).mean()  # Shorter period for 15m
    vol_ok = volatility.iloc[-1] >= 0.0005  # Even lower threshold for 15m crypto (0.05%)
    
    logger.info(f"📊 VOLATILITY: {volatility.iloc[-1]*100:.3f}% (min: 0.05%)")
    
    if not vol_ok:
        logger.info(f"❌ VOLATILITY REJECTED - Too low")
        return False

    # 15M OPTIMIZED position filter - much more permissive
    recent_highs = highs.tail(15).max()  # Shorter lookback for 15m
    recent_lows = lows.tail(15).min()
    if recent_highs != recent_lows:
        current_position = (curr['close'] - recent_lows) / (recent_highs - recent_lows)
        
        logger.info(f"📍 POSITION: {current_position*100:.1f}% of 15-candle range")
        
        # Much more permissive - only avoid extreme positions
        if signal_type == 'SELL' and current_position < 0.10:  # Only avoid bottom 10%
            logger.info(f"❌ SELL POSITION REJECTED - Too low")
            return False
        if signal_type == 'BUY' and current_position > 0.90:   # Only avoid top 10%
            logger.info(f"❌ BUY POSITION REJECTED - Too high")
            return False

    # 15M OPTIMIZED momentum - much more flexible
    momentum_3 = (curr['close'] - df.iloc[-3]['close']) / df.iloc[-3]['close']  # Shorter period for 15m
    
    logger.info(f"🚀 MOMENTUM: 3-candle: {momentum_3*100:.3f}%")
    
    if signal_type == 'BUY' and momentum_3 < -0.008:  # Allow negative momentum up to -0.8%
        logger.info(f"❌ BUY MOMENTUM REJECTED - Too negative")
        return False
    if signal_type == 'SELL' and momentum_3 > 0.008:  # Allow positive momentum up to +0.8%
        logger.info(f"❌ SELL MOMENTUM REJECTED - Too positive")
        return False

    # 15M OPTIMIZED range filter - much more permissive
    price_range = (highs.tail(15).max() - lows.tail(15).min()) / closes.tail(15).mean()  # Shorter period
    
    logger.info(f"📊 RANGE: 15-candle range: {price_range*100:.2f}%")
    
    if price_range < 0.004:  # Much lower threshold for 15m - avoid only dead markets
        logger.info(f"❌ RANGE REJECTED - Too flat")
        return False
    if price_range > 0.25:    # Much higher threshold - allow very volatile markets
        logger.info(f"❌ RANGE REJECTED - Too volatile") 
        return False

    # 15M OPTIMIZED volume confirmation - more permissive
    avg_volume = df['volume'].rolling(10).mean().iloc[-1]  # Shorter period for 15m
    volume_ok = curr['volume'] >= avg_volume * 0.8  # Only require 80% of average
    
    logger.info(f"📊 FINAL VOLUME: {(curr['volume']/avg_volume)*100:.1f}% of 10-period avg")
    
    if not volume_ok:
        logger.info(f"❌ FINAL VOLUME REJECTED")
        return False

    logger.info(f"✅ ALL 15M FILTERS PASSED for {signal_type}")
    return True

def generate_signal(symbol, df, timeframe):
    """15M OPTIMIZED signal generation with comprehensive logging"""
    logger.info(f"🎯 === 15M SIGNAL CHECK: {symbol} {timeframe} ===")
    
    curr = df.iloc[-1]
    logger.info(f"🔍 Current State - RSI9: {curr['rsi_9']:.1f}, RSI21: {curr['rsi_21']:.1f}, Vol: {curr['volume_ratio']:.2f}")
    
    # First check: RSI crossover
    signal_type = detect_rsi_crossover(df)
    if not signal_type:
        logger.info(f"❌ {symbol} {timeframe}: No RSI crossover detected")
        return None
    
    logger.info(f"✅ RSI CROSSOVER FOUND: {signal_type}")
    ...
    # IMPROVED market condition filter - block problematic market conditions
    if market_info['condition'] == 'CHOPPY':
        logger.info(f"❌ {symbol} {timeframe}: {signal_type} signal blocked - CHOPPY market detected")
        return None
    
    # Block low volatility MIXED markets - they show 20% win rate
    if market_info['condition'] == 'MIXED' and market_info['volatility_pct'] < 0.6:
        logger.info(f"❌ {symbol} {timeframe}: {signal_type} signal blocked - Low volatility MIXED market (vol: {market_info['volatility_pct']:.2f}%)")
        return None
        
    # Also block very high volatility MIXED markets (dangerous conditions)
    if market_info['condition'] == 'MIXED' and market_info['volatility_pct'] > 2.5:
        logger.info(f"❌ {symbol} {timeframe}: {signal_type} signal blocked - High volatility MIXED market (vol: {market_info['volatility_pct']:.2f}%)")
        return None

    curr = df.iloc[-1]
    entry_price = curr['close']

    # Dynamic TP/SL based on market conditions for better performance
    if market_info['condition'] == 'TRENDING':
        # Trending markets: Use full targets
        tp_percent = settings.TP_PERCENT  # 0.75%
        sl_percent = settings.SL_PERCENT  # 0.40%
    elif market_info['condition'] == 'MIXED' and market_info['volatility_pct'] >= 0.6:
        # High volatility mixed markets: More conservative targets
        tp_percent = 0.005  # 0.50% TP (reduced)
        sl_percent = 0.003  # 0.30% SL (reduced)
        logger.info(f"📊 Using conservative targets for MIXED market: TP {tp_percent*100:.2f}%, SL {sl_percent*100:.2f}%")
    else:
        # Default (should not reach here due to filtering above)
...
def format_signal_message(signal):
    ...
    # Calculate actual TP/SL percentages from prices
    entry = signal['entry_price']
    tp_pct = abs((signal['tp'] - entry) / entry * 100)
    sl_pct = abs((signal['sl'] - entry) / entry * 100)
    
    return (
        f"🚀 [{signal['symbol']}] {signal['type']} SIGNAL\n"
        f"💰 Entry: {signal['entry_price']:.6f}\n"
        f"🎯 TP: {signal['tp']:.6f} (+{tp_pct:.2f}%)\n"
